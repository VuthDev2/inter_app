...
class WhisperSpeechService implements SpeechServiceInterface {
  ...
  async startListening(
    ...
    const recorder = new AudioModule.AudioRecorder({
      ...
      android: {
        ...
        // Routes capture through Android's voice-call audio path, which
        // applies the OS's built-in echo cancellation, noise suppression,
        // and gain control — the native counterpart to the web client's
        // MediaTrackConstraints noiseSuppression: true.
  ...
  private pollLevel(turnId: number): void {
    ...
    if (this.heardSpeech) {
      const silenceMs = this.lastLoudAt ? now - this.lastLoudAt : -1;
      if (__DEV__ && elapsed % 500 < POLL_INTERVAL_MS) {
        console.log(`[QuickVoice VAD] polling: level=${level.toFixed(1)} silenceMs=${silenceMs} elapsed=${elapsed}`);
      }
      if (this.lastLoudAt && silenceMs >= TRAILING_SILENCE_MS) {
        if (__DEV__) console.log(`[QuickVoice VAD] trailing silence reached (${silenceMs}ms), finishing turn`);
        void this.finishTurn(turnId);
        return;
      }
      if (elapsed >= MAX_UTTERANCE_MS) {
        if (__DEV__) console.log("[QuickVoice VAD] max utterance reached, finishing turn");
        void this.finishTurn(turnId);
      }
      return;
    }
  ...
  private async finishTurn(turnId: number): Promise<void> {
    if (__DEV__) console.log(`[QuickVoice VAD] finishTurn called (turnId=${turnId}, current=${this.turnId}, finishing=${this.finishing})`);
    ...
    if (!recorder) {
      if (__DEV__) console.log("[QuickVoice VAD] finishTurn: no recorder, bailing");
      return;
    }
    ...
    try {
      await recorder.stop();
      uri = recorder.uri;
      if (__DEV__) console.log(`[QuickVoice VAD] recorder stopped, uri=${uri}`);
    } catch (stopError) {
      if (__DEV__) console.log("[QuickVoice VAD] recorder.stop() threw:", stopError);
    ...
    if (!uri) {
      if (__DEV__) console.log("[QuickVoice VAD] no uri after stop, emitting no_speech");
      if (turnId === this.turnId) emitError({ code: "no_speech", message: "" });
      return;
    }

    try {
      if (__DEV__) console.log("[QuickVoice VAD] uploading to /transcribe...");
      const result = await transcribeAudioResult(uri, this.languageHint, this.expectedLanguage);
      if (__DEV__) console.log("[QuickVoice VAD] transcribe result:", JSON.stringify(result));
      ...
      if (__DEV__) console.log("[QuickVoice VAD] transcribe upload threw:", reason);
      if (turnId !== this.turnId) return;
      emitError({
