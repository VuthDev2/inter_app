    ...
    return (
        <div className="min-h-screen bg-[#0a0e1a] text-white flex flex-col font-sans relative">
            ...
            <div className="flex-1 flex flex-col px-6 pt-10 pb-40 relative z-10 w-full max-w-[1200px] mx-auto">
                ...
                <div className="text-[13px] text-[#3b82f6] font-medium mb-8">
                    1 way conversations
                </div>
                        ...
                        {SEGMENTS.map((segment, index) => {
                            ...
                            return (
                                <div key={segment.id} className={`flex flex-col px-8 py-6 ${index !== SEGMENTS.length - 1 ? 'border-b border-white/5' : ''}`}>
                                    <div 
                                        className="flex items-center gap-2 text-[#3b82f6] mb-4 cursor-pointer w-max hover:text-blue-400 transition-colors"
                                    ...
                                    <div className="flex flex-col gap-5 pl-1 relative">
                                        <div className="absolute left-0 top-1 bottom-1 w-[2px] bg-white/10 rounded-full"></div>
                                        
                                        {isExpanded && (
                                            <div className="relative">
                                                <div className="absolute -left-1 top-0 bottom-0 w-[2px] bg-white rounded-full z-10"></div>
                                                <p className="text-[15px] leading-relaxed text-white/90 pl-5">{segment.source}</p>
                                            </div>
                                        )}
                                        
                                        <div className="relative">
                                            <div className={`absolute -left-1 top-0 bottom-0 w-[2px] rounded-full z-10 ${isExpanded ? 'bg-[#3b82f6]/40' : 'bg-[#3b82f6]'}`}></div>
                                            <p className={`text-[15px] leading-relaxed pl-5 ${isExpanded ? 'text-white/60' : 'text-white/90'}`}>
                                                {segment.translation}
                                            </p>
                                        </div>
                                    </div>
